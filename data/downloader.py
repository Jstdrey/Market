...
def build_dataframe(candles, start_ms, end_ms, symbol, timeframe):  # Создаем функцию, которая превращает список свечей в таблицу Pandas.
    ...
    df = df[["timestamp", "datetime", "symbol", "timeframe", "open", "high", "low", "close", "volume"]]  # Переставляем колонки в удобный порядок.
...
def run_downloader(symbol, timeframe, start_date, end_date, output_file=OUTPUT_FILE):  # Создаем функцию, которую можно вызывать из интерфейса с выбранными пользователем параметрами.
    ...
    df = build_dataframe(candles=candles, start_ms=start_ms, end_ms=end_ms, symbol=symbol, timeframe=timeframe)  # Превращаем список свечей в таблицу Pandas и фиксируем метаданные набора.
