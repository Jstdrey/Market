...
OLD_CSV_WARNING_TEXT = "Файл data.csv не содержит метаданных symbol/timeframe. Нажмите 'Загрузить данные с Binance', чтобы пересоздать файл и исключить путаницу данных."  # Сохраняем единый текст предупреждения для старого формата CSV без метаданных.
...
def render_sidebar():  # Создаем функцию, которая рисует боковую панель с настройками пользователя.
    ...
    if isinstance(selected_date_range, date):  # Проверяем, не вернул ли Streamlit одиночную дату вместо диапазона.
        selected_date_range = [selected_date_range]  # Преобразуем одиночную дату в список, чтобы безопасно обработать ниже.
    ...
    if not isinstance(selected_date_range, list) or len(selected_date_range) != 2:  # Проверяем, выбрал ли пользователь обе границы диапазона.
...
def show_dataset_mismatch_warning(dataframe, selected_symbol, selected_timeframe):  # Создаем функцию, которая предупреждает о несоответствии выбранных параметров и текущего data.csv.
    ...
    if dataset_symbol is None or dataset_timeframe is None:  # Проверяем, есть ли в файле метаданные новой версии загрузчика.
        st.warning(OLD_CSV_WARNING_TEXT)  # Показываем предупреждение для старого формата файла.
...
def handle_data_button(selected_symbol, selected_timeframe, selected_start_date, selected_end_date):  # Создаем функцию, которая обрабатывает нажатие кнопки загрузки данных.
    ...
    if button_clicked:  # Проверяем, была ли нажата кнопка.
        ...
        try:  # Начинаем блок безопасного запуска загрузчика, чтобы ошибки API не роняли интерфейс целиком.
        ...
        except Exception as error:  # Ловим все ошибки загрузки и показываем понятное сообщение пользователю.
            st.error(f"Не удалось загрузить данные: {error}")  # Показываем текст ошибки, чтобы пользователь понимал причину сбоя.
...
def show_backtest_tab(available_strategies, selected_symbol, selected_timeframe):  # Создаем функцию, которая выводит вкладку бектеста и запускает симуляцию по кнопке.
    ...
    if button_clicked:  # Проверяем, нажал ли пользователь кнопку запуска.
        try:  # Начинаем блок безопасного запуска бектеста, чтобы возможные ошибки данных отображались в интерфейсе.
        ...
        except Exception as error:  # Ловим ошибки движка бектеста, чтобы приложение не падало.
            st.error(f"Ошибка запуска бектеста: {error}")  # Показываем пользователю текст ошибки для быстрой диагностики.
...
def show_optimization_tab(available_strategies, selected_symbol, selected_timeframe):  # Создаем функцию, которая выводит вкладку оптимизации и запускает перебор параметров по кнопке.
    ...
    if button_clicked:  # Проверяем, нажал ли пользователь кнопку запуска оптимизации.
        try:  # Начинаем блок безопасного запуска оптимизации, чтобы ошибки не ломали весь интерфейс.
        ...
        except Exception as error:  # Ловим ошибки оптимизации, чтобы отдать их пользователю в понятном виде.
            st.error(f"Ошибка запуска оптимизации: {error}")  # Показываем текст ошибки оптимизации в интерфейсе.
